...
### 4. Markdown・数式・表を表示する

エージェントの回答はMarkdownとして表示されます。見出し、箇条書き、リンク、引用、コードブロック、Markdown表に対応しています。LaTeX数式は、インラインなら `$E=mc^2$`、ブロックなら `$$\int_0^1 x^2 dx$$` または `\[ ... \]` の形式で記述してください。描画ライブラリはアプリに同梱されるため、オフラインでも表示できます。コードブロック内の文字列は数式として解釈しません。

### 5. メディアとURLをエージェントへ渡す
